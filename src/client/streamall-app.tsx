...
function queueItemMeta(item: PlayableItem, library: LibrarySnapshot) {
  const artist = itemLabel(item, library).artist;
  if (item.kind === "track" && item.albumId) {
    ...
    return `${artist} · ${album?.title ?? "Album"} · ${track}`;
  }
  return artist;
}
